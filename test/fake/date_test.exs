defmodule Fake.DateTest do
  use ExUnit.Case, async: true

  doctest Fake.Date

  test "date_of_birth/0" do
    assert age(Fake.Date.date_of_birth()) in 18..99
  end

  test "date_of_birth/1 with exact age" do
    assert age(Fake.Date.date_of_birth(1)) == 1
  end

  test "date_of_birth/1 with age range" do
    assert age(Fake.Date.date_of_birth(12..99)) in 12..99
  end

  test "forward/1" do
    forwarded_date = Fake.Date.forward(10)
    assert %Date{year: year, month: month, day: day} = forwarded_date
    assert now().year < year || now().month < month || now().day < day
  end

  test "backward/1" do
    backward_date = Fake.Date.backward(10)
  ...
  test "between/2" do
    ...
    between_date = Fake.Date.between(from_date, to_date)
