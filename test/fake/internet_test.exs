defmodule Fake.InternetTest do
  use ExUnit.Case, async: true

  import Fake.Internet

  doctest Fake.Internet
  doctest Fake.Internet.En
  doctest Fake.Internet.Es
  doctest Fake.Internet.Hy
  doctest Fake.Internet.It
  doctest Fake.Internet.PtBr

  @iterations 10_000

  test "ip_v4_address/0" do
    assert Regex.match?(~r/^(\d+\.){3}\d+$/, Fake.Internet.ip_v4_address())
  end

  test "ip_v6_address/0" do
    assert Regex.match?(~r/^([0-9A-F]{4}:){7}[0-9A-F]{4}$/, Fake.Internet.ip_v6_address())
  end

  test "mac_address/0" do
    assert Regex.match?(~r/^([0-9a-f]{2}:){5}[0-9a-f]{2}$/, Fake.Internet.mac_address())
  end

  test "url/0" do
    assert Regex.match?(~r/^http[s]?:\/\/\w+\.\w+$/, Fake.Internet.url())
  end

  test "slug/0" do
    assert Regex.match?(~r/\A([a-z0-9]+[-\._])*[a-z0-9]+\z/, Fake.Internet.slug())
  end

  test "slug/1" do
    slug = Fake.Internet.slug(["hello", "world"])
