defmodule Fake do
  ...
  @doc """
  Starts Fake with default locale.
  """
  @spec start() :: :ok
  def start do
    :application.start(:fake)
  end

  @doc """
  Starts Fake with `lang` locale.
  """
  @spec start(atom) :: :ok
  ...
  @spec locale() :: atom
  def locale do
    Application.get_env(:fake, :locale)
  end
  ...
  @spec country() :: atom
  def country do
    Application.get_env(:fake, :country)
  end
  ...
  def locale(lang) when is_atom(lang) do
    Application.put_env(:fake, :locale, lang)
  end
  ...
  def random_uniform() do
    Application.get_env(:fake, :random_module).random_uniform()
  end
  ...
  def random_between(left, right) do
    Application.get_env(:fake, :random_module).random_between(left, right)
  end
  ...
  def random_bytes(total) do
    Application.get_env(:fake, :random_module).random_bytes(total)
  end
  ...
  def shuffle(enum) do
    Application.get_env(:fake, :random_module).shuffle(enum)
  end
  ...
  defmacro sampler(name, data) do
    ...
    quote do
      def unquote(name)() do
        unquote(mapped_data)
        |> Map.get(Fake.random_between(0, unquote(count - 1)))
      end
    end
  end
end
