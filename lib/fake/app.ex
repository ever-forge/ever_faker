defmodule Fake.App do
  ...
  * `:allow_build` - when `true`, a build identifier (e.g.: `+001`)
  ...
  def semver(opts \\ []) do
    ...
    formats
    |> Enum.at(Fake.random_between(0, length(formats) - 1))
    |> Fake.format()
  ...
  defp semver_reformat_bad_luck(<<"0.0", rest::binary>>) do
    semver_reformat_bad_luck(<<"0.#{Fake.random_between(1, 9)}", rest::binary>>)
  end
  ...
  defp semver_reformat_bad_luck(
    ...
    semver_reformat_bad_luck(
      <<first, ".", second, third, ".#{Fake.random_between(1, 9)}", rest::binary>>
    )
  end
  ...
  def author, do: author(Fake.random_between(0, 1))
